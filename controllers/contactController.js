...
async function searchContact(req, res) {
	...
	if (searchedPhoneNumber && !helper.validatePhoneNumber(searchedPhoneNumber)) {
...
async function addNewContact(req, res) {
	...
	if (
		...
		res
			.status(400)
			.json({ error: 'Invalid Contact Name/Contact Phone Number.' });
		return;
	}
	...
	try {
		...
		if (!existingUser) {
			...
			if (existingSpam) {
				res
					.status(400)
					.json({ error: 'You have already marked this user as spam.' });
			}
